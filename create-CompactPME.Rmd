...
usethis::create_package(
  ...
  fields = list(
    ...
    Description = paste(
      "Estimates smooth principal curves and surfaces from noisy point",
      "clouds using compact one- and two-dimensional Riemannian template",
      "manifolds. Implementations are provided for interval, circle, and",
      "sphere templates, including projection-adaptation fitting and",
      "smoothing-parameter selection utilities."
    ),
    `Authors@R` = person(
      given = "Christopher",
      family = "Perez",
    ...
    Author = "Christopher Perez [aut, cre]",
    Maintainer = "Christopher Perez <cmp23b@fsu.edu>",
    URL = "https://github.com/cmperez024/Compact-Principal-Manifold-Estimation",
    BugReports = paste0(
      "https://github.com/cmperez024/",
      "Compact-Principal-Manifold-Estimation/issues"
    )
  )
)
...
There are two aspects of projection. First, there is an initialization projection. This is acquired using the circular projection method. We center the data $\{X_i\}_{i=1}^N \subset \mathbb{R}^D$ and then deduce each points angle along a circle. The second involves calculating the projection indices $\pi_f$ relative to a function $f$. The latter function constructs a partition (which we refer to as a grid in the code) and calculates the minimizing partition element $t \in \mathcal{P}[0,1]$ such that $\Vert X_i - f(t) \Vert$ is minimized. This is done for each point $X_i$ The optimize routine uses built-in functions to minimize the norm difference over $t$.
...
Kernel smoothing is a step imperative for our smoothing parameter lection. The general steps are to do a kernel regression on the squared residuals by the projection index value. Then, we generate predictions for the residual values at new projection points to generate a dense grid of residuals. We can then calculate summary statistics of these residuals, for each $\lambda$. In particular, we minimize the coefficient of variation.
```{r}
